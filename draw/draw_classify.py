...
class AttentionReader(Initializable):
    ...
    @application(inputs=['x', 'c'], outputs=['r'])
    def apply(self, x, c):
...
class DrawClassifyModel(BaseRecurrent, Initializable, Random):
    ...
    def get_dim(self, name):
        if name == 'c':
            # return self.reader.get_dim('x_dim') # not true in this case, should be dim of y
            return 10
        elif name == 'h_enc':
    ...
    @recurrent(sequences=['u'], contexts=['x'],
               states=['c', 'h_enc', 'c_enc', 'h_dec'],
               outputs=['c', 'h_enc', 'c_enc', 'z', 'kl', 'h_dec'])
    def apply(self, u, c, h_enc, c_enc, h_dec, x):
        r = self.reader.apply(x, c)
        ...
        h_dec, c = self.decoder_rnn.apply(states=h_dec, cells = c, inputs=i_dec, iterate=False)
        c = T.nnet.softmax(T.nnet.sigmoid(c))

        return c, h_enc, c_enc, z, kl, h_dec

    @recurrent(sequences=['u'], contexts=[],
               states=['c', 'h_dec'],
               outputs=['c', 'h_dec'])
    def decode(self, u, c, h_dec):
        batch_size = c.shape[0]

        z = self.sampler.sample_from_prior(u)
        i_dec = self.decoder_mlp.apply(z)
        h_dec, c = self.decoder_rnn.apply(
            states=h_dec, cells=c,
    ...
    @application(inputs=['features'], outputs=['c', 'kl'])
    def classify(self, features):
        ...
        c, h_enc, c_enc, z, kl, h_dec = self.apply(x=features, u=u)
