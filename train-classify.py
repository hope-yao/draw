...
from draw.draw_classify import *
...
def main(name, dataset, epochs, batch_size, learning_rate, attention,
    ...
    encoder_mlp = MLP([Identity()], [(x_dim + dec_dim), 4 * enc_dim], name="MLP_enc", **inits)
    ...
    y_hat, kl_terms = draw.classify(x)
    ...
    classification_error = -T.mean(T.log(y_hat_last)[T.arange(100), y_int]) # guess (rnn_iter (16), class (10), batch_size)
    classification_error.name = "classification_error"

    recognition_convergence = (-y_hat*T.log2(y_hat)).sum(axis=1).mean()
    recognition_convergence.name = "recognition_convergence"

    cost = classification_error + kl_terms.sum(axis=0).mean() + recognition_convergence.mean()
    cost.name = "cost"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enc-dim", type=int, dest="enc_dim",
                        default=10, help="Encoder RNN state dimension") # originally 256
    ...
    parser.add_argument("--z-dim", type=int, dest="z_dim",
                        default=5, help="Z-vector dimension") # originally 100
